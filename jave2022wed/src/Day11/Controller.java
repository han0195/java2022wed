...
public class Controller {
	...
	//4.수정메소드
	public static boolean update(int index, String pw, String tit, String con) {
	
		if(boadlist.get(index).getPassword().equals(pw)) {
			boadlist.get(index).setTitle(tit);
			boadlist.get(index).setContent(con);
			save();
			return true;
		}	
		return false;
	...
	//5.삭제메소드 
	public static void delete(int index, String pw) {
		if(boadlist.get(index).getPassword().equals(pw)) {
	...
	//6.댓글쓰기메소드 
	public static void replywrite(int index, String rcon, String rpw, String rwr) {
	...
	public static void save() { 		// 리스트내 모든 게시물을 파일에 저장
		try {
			// 1. 파일출력 클래스 
			FileOutputStream outputStream = new FileOutputStream("C:/Users/504/Desktop/게시물파일.txt");
	...
	public static void load() {
		try {
			FileInputStream inputStream = new FileInputStream("C:/Users/504/Desktop/게시물파일.txt"); // 1. 파일입력클래스 
